...
// - Saat create QR: kita "reserve" dulu kuota (token TTL). Ini mencegah race & spam.
// - Saat PAID: token di-commit => kuota benar-benar terpakai.
// - Saat CANCEL/EXPIRED/FAILED: token di-release => kuota balik.
//
// Catatan:
...
const UNLIMITED_DEVICE_KEYS = (process.env.UNLIMITED_DEVICE_KEYS || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
