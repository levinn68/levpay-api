...
// ✅ deviceKey ini jadi UNLIMITED (bypass limit 1x/bulan, tapi tetap kena reserved TTL biar anti-spam)
const UNLIMITED_DEVICE_KEYS = new Set([
  "16e2ded7e826d92de785c205451d79c3e608da005f4bf7f8097e360c134f3e54",
]);
...
function ensure(db) {
  ...
  // seed unlimited keys (tidak mengubah mekanik diskon, hanya bypass limit bulanan utk key tertentu)
...
function applyDiscount({ db, amount, deviceKey, voucherCode, reserveTtlMs = 6 * 60 * 1000 }) {
  ...
  // reserve monthly setelah voucher
...
function adminSetMonthlyPromo(db, body) {
  ...
  // optional: manage unlimited list via curl (kalau mau)
